...
async function fetchTotalCount() {
  const resp = await fetch("api/papers?search=");
  return resp.json();
}
 ...
 * Each group has at most one of each type.
 */
...
function renderGroupCard(g) {
  ...
  const viewSvg = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
  const dlSvg = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>';

  const buttonPairs = [
    { obj: g.qp, type: 'qp', label: 'QP', labelFull: 'Question Paper', cls: 'type-qp' },
    { obj: g.ms, type: 'ms', label: 'MS', labelFull: 'Marking Scheme', cls: 'type-ms' },
    { obj: g.er, type: 'er', label: 'ER', labelFull: 'Examiner Report', cls: 'type-er' },
    { obj: g.ir, type: 'ir', label: 'IR', labelFull: 'Investigator Report', cls: 'type-ir' },
    { obj: g.in, type: 'in', label: 'IN', labelFull: 'Insert', cls: 'type-in' },
  ];
  ...
  for (const pair of buttonPairs) {
    if (pair.obj) {
      buttons += `
        <button class="btn btn-outline btn-sm preview-btn" data-path="${pair.obj.relative_path}" data-title="${code} — ${pair.labelFull}">
          ${viewSvg} View ${pair.label}
        </button>
        <a class="btn btn-primary btn-sm" href="api/download/${encodeURI(pair.obj.relative_path)}">
          ${dlSvg} Download ${pair.label}
        </a>`;
      badges += `<span class="type-badge ${pair.cls}">${pair.label}</span>`;
    }
  }
...
function openPreview(path, title) {
  ...
  // 联动 AI 聊天: 传递当前试卷上下文
  if (window.AmosChat) {
    window.AmosChat.setContext(title);
  }
}
...
function closePreview() {
  ...
  // 清除上下文
  if (window.AmosChat) {
    window.AmosChat.setContext(null);
  }
}
...
async function init() {
  ...
  try {
    const allPapers = await fetchTotalCount();
    const total = allPapers.length;
    if (pageDesc && total > 0) {
      pageDesc.textContent = `Browse and download past papers with marking schemes — ${total.toLocaleString()} CIE papers`;
    }
  } catch (e) {
