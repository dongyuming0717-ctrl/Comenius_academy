...
class FewShotBank:
    def __init__(self):
        os.makedirs(DATA_DIR, exist_ok=True)
        self.examples: dict[str, list[dict]] = self._load()

    def _load(self) -> dict:
        if FEWSHOT_PATH.exists():
            return json.loads(FEWSHOT_PATH.read_text())
        return {}

    def save(self):
        FEWSHOT_PATH.write_text(json.dumps(self.examples, ensure_ascii=False, indent=2))

    def add_correct(self, topic: str, question: str, amos_answer: str, answer_key: str):
        if topic not in self.examples:
            self.examples[topic] = []
        self.examples[topic].append({
            "question": question[:600],
            "solution": amos_answer[:600],
            "answer": answer_key,
        })
        if len(self.examples[topic]) > 5:
            self.examples[topic] = self.examples[topic][-5:]
        self.save()

    def get_examples(self, topic: str, k: int = 2) -> list[dict]:
        examples = self.examples.get(topic, [])
        return examples[-k:] if examples else []
    ...
    def get_context_prompt(self, topic: str, k: int = 2) -> str:
        ...
        for i, ex in enumerate(examples):
            ...
            parts.append(f"解答: {ex['solution'][:300]}")
        return "\n".join(parts)
