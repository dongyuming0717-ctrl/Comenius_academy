        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(520px, 1fr))', gap: 24 }}>
          {TESTS.map((test) => (
            <div key={test.id} style={{
              ...
              transform: hoveredId === test.id ? 'translateY(-4px)' : 'translateY(0)',
              boxShadow: hoveredId === test.id ? '0 20px 25px rgba(0,0,0,0.1)' : 'none',
              borderColor: hoveredId === test.id ? colors.primary : 'rgba(226,232,240,0.5)',
              transition: 'all 0.25s ease',
            }} onMouseEnter={() => setHoveredId(test.id)} onMouseLeave={() => setHoveredId(null)}>
              <div style={cardHeader}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 12 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                      <h3 style={{ margin: 0, fontSize: 24, fontWeight: 600, color: colors.foreground }}>{test.name}</h3>
                      <span style={test.difficulty === 'Very Hard' ? badgeVeryHard : badgeHard}>{test.difficulty === 'Very Hard' ? t('admission.veryHard') : t('admission.hard')}</span>
                    </div>
                    <p style={{ margin: 0, fontSize: 16, color: colors.mutedForeground }}>{test.fullName}</p>
                  </div>
                </div>
                <p style={{ margin: 0, fontSize: 14, color: colors.mutedForeground }}>{test.description}</p>
              </div>
              <div style={cardContent}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, fontSize: 14, marginBottom: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ color: colors.mutedForeground }}>{test.papers} {t('admission.pastPapers')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={colors.mutedForeground} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                    </svg>
                    <span style={{ color: colors.mutedForeground }}>{test.duration}</span>
                  </div>
                </div>
                <button
                  style={{ ...btnPrimary, width: '100%', justifyContent: 'center' }}
                  onClick={() => navigate(`/admission-tests/${test.id}`)}
                >
                  {t('admission.viewPastPapers')}
        ...
        <div style={{ marginTop: 32, ...cardStyle, borderColor: 'rgba(30,64,175,0.5)', background: 'rgba(219,234,254,0.3)' }}>
          <div style={cardHeader}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: colors.foreground }}>{t('admission.whyPractice')}</h3>
            </div>
          </div>
          <div style={cardContent}>
            <p style={{ margin: '0 0 12px', fontSize: 14, color: colors.mutedForeground }}>
              {t('admission.whyPracticeDesc')}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 14 }}>
              {[
                t('admission.feature1'),
                t('admission.feature2'),
                t('admission.feature3'),
                t('admission.feature4'),
              ].map((featureText) => (
                <div key={featureText} style={{ display: 'flex', gap: 8 }}>
                  <span style={{ color: colors.primary }}>•</span>
                  <span style={{ color: colors.mutedForeground }}>{featureText}</span>
                </div>
              ))}
            </div>
          </div>
...
function TestDetail() {
    ...
    return (
      <div style={pageContainer}>
        ...
        <div style={{ ...container, textAlign: 'center' }}>
          <h2 style={{ color: colors.mutedForeground }}>{t('admission.testNotFound')}</h2>
          <Link to="/admission-tests" style={{ color: colors.primary }}>{t('admission.backToTests')}</Link>
        </div>
      </div>
    );
  }
  ...
  const statCards = [
    { label: t('admission.mocksTaken'), value: userStats ? userStats.mocksTaken : 0 },
    { label: t('admission.questionsDone'), value: userStats ? userStats.questionsDone : 0 },
    { label: t('admission.averageAccuracy'), value: userStats ? `${userStats.avgAccuracy}%` : '0%' },
    { label: t('admission.averageScore'), value: userStats ? `${userStats.avgScore}/20` : '0/20' },
  ];
    ...
    <div style={pageContainer}>
      ...
      <div style={container}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ margin: 0, fontSize: 18, color: colors.mutedForeground }}>
            {test.fullName} — {papers.length || test.papers} {t('admission.pastPapersAvailable')}
          </p>
        </div>
          ...
          <>
            ...
            <div style={{
              ...
              <h2 style={{
                ...
                {t('admission.examSimulatorDemo')}
              </h2>
              <p style={{
                margin: '0 0 16px 0', fontSize: 13, color: '#888',
                fontFamily: font,
              }}>
                {t('admission.demoDesc')}
              </p>
              <video
            ...
            <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
              ...
              <button style={{ ...btnPrimary, flex: 1, justifyContent: 'center', padding: '16px 24px', fontSize: 16, fontWeight: 600 }} onClick={() => navigate('/random')}>
                ...
                {t('admission.generate')}
              </button>
              <button style={{ ...btnPrimary, flex: 1, justifyContent: 'center', padding: '16px 24px', fontSize: 16, fontWeight: 600 }} onClick={() => setShowPDFModal(true)}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
                </svg>
                {t('admission.pastPaper')}
              </button>
            </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  <div style={{
                    ...
                    <div style={{ marginBottom: 12 }}>
                      <span style={tagStyle('#eff6ff', '#1e40af')}>{t('admission.generatedTag')}</span>
                    </div>
                    <h3 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600, color: '#333' }}>
                      {generatedPaper.title}
                    </h3>
                    <div style={{ fontSize: 12, color: '#888', marginBottom: 12 }}>
                      {generatedPaper.duration_minutes} {t('mcqFullPapers.min')} &middot; {(generatedPaper.questions as Question[]).length} {t('mcqChapters.questions')}
                    </div>
                    <button
        ...
        {isTMUA && (
          <h2 style={{
            ...
            {t('admission.availableMockExams')}
          </h2>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', margin: '0 0 16px 0' }}>
          <div style={{ fontSize: 14, color: colors.mutedForeground }}>
            {loading ? t('admission.loading') : `${papers.length} ${t('admission.pastPapersAvailable')}`}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          {loading && (
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 48, color: colors.mutedForeground }}>{t('admission.loadingPapers')}</div>
          )}
            ...
            return (
              <div key={p.id} style={{
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                  <span style={tagStyle('#eff6ff', '#1e40af')}>{t('admission.pastPapersTag')}</span>
                  <span style={p.paper_number === 1
                    ? tagStyle('#f0fdf4', '#16a34a')
                    : tagStyle('#fef3c7', '#d97706')
                  }>
                    {t('admission.paper')} {p.paper_number}
                  </span>
                </div>
                ...
                <div style={{
                  ...
                  <span>{p.duration_minutes} {t('mcqFullPapers.min')}</span>
                  <span>{qCount} {t('admission.questionsShort')}</span>
                </div>
                ...
                {stats && (
                  <div style={{
                    ...
                    <div>
                      <span style={{ color: '#888' }}>{t('admission.accuracy')}: </span>
                      <strong>{stats.accuracy}%</strong>
                    </div>
                    <div>
                      <span style={{ color: '#888' }}>{t('admission.completion')}: </span>
                      <strong>{stats.completionRate}%</strong>
                    </div>
                    {stats.score !== null && (
                      <div>
                        <span style={{ color: '#888' }}>{t('admission.score')}: </span>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button
                    onClick={() => { if (hasRecord) navigate('/exam', { state: { paperId: p.id, viewLatestRecord: true } }); }}
                    disabled={!hasRecord}
                    style={{
          ...
          {!loading && testId !== 'tmua' && (
            <div style={{ textAlign: 'center', padding: 48, color: colors.mutedForeground }}>
              <p style={{ fontSize: 18, fontWeight: 500, marginBottom: 8 }}>{t('admission.comingSoon')}</p>
              <p>{t('admission.comingSoonDesc').replace('{test}', test?.name || '')}</p>
              <p style={{ marginTop: 8 }}>{t('admission.checkBack')} <Link to="/past-papers" style={{ color: colors.primary }}>{t('admission.pastPaperLibrary')}</Link>.</p>
            </div>
          )}
          {!loading && testId === 'tmua' && sortedPapers.length === 0 && (
            <div style={{ textAlign: 'center', padding: 48, color: colors.mutedForeground }}>
              {t('admission.noPapers')}
            </div>
          )}
        </div>
      </div>
        ...
        >
          ...
          >
            <div style={{
              ...
              <span style={{ fontWeight: 600, fontSize: 18, color: '#333' }}>{t('admission.pdfModalTitle')}</span>
              <button
                onClick={() => setShowPDFModal(false)}
                style={{
