...
export function ProctorProvider({ children }: { children: ReactNode }) {
  ...
  const lastReportedTime = useRef<Partial<Record<ViolationType, number>>>({});
  const sensitivityConfig = useRef<SensitivityConfig>(DEFAULT_SENSITIVITY);
  ...
  const reportViolation = useCallback((type: ViolationType, detail?: string) => {
    const now = Date.now();
    const cfg = sensitivityConfig.current;
    const cooldown = cfg.cooldownMs[type] ?? cfg.defaultCooldownMs;
    const last = lastReportedTime.current[type];
    if (last !== undefined && now - last < cooldown) return;

    lastReportedTime.current[type] = now;
